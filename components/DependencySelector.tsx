...
const DependencySelector: React.FC<DependencySelectorProps> = ({ allWidgets, currentWidgetId, value, onChange }) => {
  ...
  return (
    <div className="flex items-center gap-1 text-xs w-full">
      <select
        ...
        className="bg-transparent text-xs rounded-md p-1 no-drag hover:bg-black/5 dark:hover:bg-white/5 focus:outline-none w-full"
      ...
      {value?.widgetId && availableDataKeys.length > 0 && (
            ...
            className="bg-transparent text-xs rounded-md p-1 no-drag hover:bg-black/5 dark:hover:bg-white/5 focus:outline-none"
